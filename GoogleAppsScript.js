...
function doGet(e) {
    try {
        PropertiesService.getScriptProperties().setProperty('firstVar', e.parameter.arg);
    }
    catch (e) {
    }
    try {
        PropertiesService.getScriptProperties().setProperty('meetingDate', e.parameter.date);
    }
    catch (e) {
    }
    try {
        PropertiesService.getScriptProperties().setProperty('meetingOdds', e.parameter.chance);
    }
    catch (e) {
        PropertiesService.getScriptProperties().setProperty('meetingOdds', "default");
    }
    try {
        PropertiesService.getScriptProperties().setProperty('rate', e.parameter.rate);
    }
    catch (e) {
    }
    try {
        PropertiesService.getScriptProperties().setProperty('jsonData', e.parameter.data);
    }
    catch (e) {
    ...
    if (PropertiesService.getScriptProperties().getProperty('jsonData') != "default") {
        var jsonData = PropertiesService.getScriptProperties().getProperty('jsonData');
        var meeting = [];
...
function printHTML() {
    var firstVar = PropertiesService.getScriptProperties().getProperty('firstVar');
    var meetingDate = PropertiesService.getScriptProperties().getProperty('meetingDate');
    var meetingOdds = PropertiesService.getScriptProperties().getProperty('meetingOdds');
    var rate = PropertiesService.getScriptProperties().getProperty('rate');
    var jsonData = PropertiesService.getScriptProperties().getProperty('jsonData');

    return HtmlService.createHtmlOutput("<h2><table border='1'>" + "<tr><td>firstVar:</td><td>" + firstVar + "</td></tr>" + "<tr><td>meetingDate:</td><td>" + meetingDate + "</td></tr>" + "<tr><td>meetingOdds:</td><td>" + meetingOdds + "</td></tr>" + "<tr><td>rate:</td><td>" + rate + "</td></tr>" + "</table></h2>");
...
function printSpreadsheetRow(meetingObj, logString) {
    ...
    var dateTimeStamp = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
    ...
    for (var i = 0; i < meetingObj.length; i++) {
        ...
        var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(meetingObj[i][0][0]);
        if (sheet != null) {
            sheet.appendRow([dateTimeStamp,
                               ...
                               meetingObj[i][5][0], meetingObj[i][5][1], meetingObj[i][5][2],
                               meetingObj[i][6][0], meetingObj[i][6][1], meetingObj[i][6][2],
                               meetingObj[i][7][0], meetingObj[i][7][1], meetingObj[i][7][2],
            ...
            sheet = SpreadsheetApp.getActiveSpreadsheet().insertSheet(meetingObj[i][0][0]);
            // append the row to this sheet within the spreadsheet
            sheet.appendRow(["Timestamp",
                             "Meeting Date", "Chance of Rate1", "Rate1",
                             "Meeting Date", "Chance of Rate2", "Rate2",
                             "Meeting Date", "Chance of Rate3", "Rate3",
                             "Meeting Date", "Chance of Rate4", "Rate4",
                             "Meeting Date", "Chance of Rate5", "Rate5",
                             "jsonData", "Log Message"]);
            sheet.appendRow([dateTimeStamp, meetingObj[i][0][0], meetingObj[i][0][1], meetingObj[i][0][2],
                             meetingObj[i][1][0], meetingObj[i][1][1], meetingObj[i][1][2],
                             /*meetingObj[i][2][0], meetingObj[i][2][1], meetingObj[i][2][2], 
                             meetingObj[i][3][0], meetingObj[i][3][1], meetingObj[i][3][2], 
                             meetingObj[i][4][0], meetingObj[i][4][1], meetingObj[i][4][2], */
