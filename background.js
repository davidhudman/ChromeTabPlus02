...
function updateBadgeOld() {
    ...
    if (needTotalInBadge == true) {
        ...
        chrome.storage.sync.get({
            ...
            if (badgeText.charAt(0) == '-') {			// negative number
    ...
    chrome.storage.sync.get({
        ...
        request.onreadystatechange = function () {
            ...
            for (var i = 0; i < tempVar.length; i++) {
                ...
                stockArray[i] = { 0: tempVar[i][0], 1: tempVar[i][1], 2: Number(tempVar[i][2]), 3: Number(tempVar[i][3]), 4: Number(tempVar[i][4]), 5: Number(0) };
            }
        ...
        spreadsheetLink = items.sheetLink.substr(0, items.sheetLink.search("edit")) + "export?" + "gid=" + items.sheetLink.substr(items.sheetLink.search("gid") + 4) + "&format=csv";
...
function requestYahooFinance(stockString) {
    ...
    request.onreadystatechange = function () {
        ...
        for (var i = 0; i < unParsedRows.length; i++) {
            ...
            if (i < unParsedRows.length - 1) {
                ...
                if (parsedRow[0].substring(1, parsedRow[0].length - 1) == stockArray[i][1]) {
                    ...
                    stockArray[i][7] = ((stockArray[i][5] - stockArray[i][6]) * stockArray[i][2]).toFixed(2);

                    // assign % profit since yesterday to a new column in stockArray
                    stockArray[i][8] = ((stockArray[i][5] - stockArray[i][6]) / stockArray[i][6] * 100).toFixed(2);

                    // assign $ profit total to a new column in stockArray
                    stockArray[i][9] = ((stockArray[i][5] * stockArray[i][2]) - stockArray[i][3]).toFixed(2);

                    // assign % profit total to a new column in stockArray
                    stockArray[i][10] = (stockArray[i][9] / stockArray[i][3] * 100).toFixed(2);
    ...
    request.open("GET", "http://finance.yahoo.com/d/quotes.csv?s=" + stockString + "&f=sbp", true);
    request.send();
}

function createTotalPurchasesArray() {

    totalValue = { 0: stockArray[0][0], 1: Number(0), 2: Number(0), 3: Number(0), 4: Number(0), 5: Number(0), 6: Number(0), 7: Number(0) };
